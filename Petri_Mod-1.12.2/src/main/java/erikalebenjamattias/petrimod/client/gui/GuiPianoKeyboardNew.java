...
@Deprecated
public class GuiPianoKeyboardNew extends GuiScreen {
    ...
    public GuiPianoKeyboardNew(EntityGrandPiano piano) {
    	this.piano = piano;
	}
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    	...
    	if(this.getKeyNumFromXY(mouseX, mouseY) > 0) {
	    	if(mouseButton == 1) {
	    		//this.keysDown[this.getKeyNumFromXY(mouseX, mouseY)] = true;
	    		this.pressKey(this.getKeyNumFromXY(mouseX, mouseY));
    ...
    protected void mouseReleased(int mouseX, int mouseY, int mouseButton) {
    	...
    	if(mouseButton == 1 && this.rightClickNote != 0) {
	    	//this.keysDown[this.rightClickNote] = false;
    		this.unpressKey(this.rightClickNote);
	    	this.rightClickNote = 0;
    	}
    	if(mouseButton == 0 && this.leftClickNote != 0) {
    		//this.keysDown[this.leftClickNote] = false;
    		this.unpressKey(this.leftClickNote);
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int mouseButton, long timeSinceLastClick) {
    	if(mouseButton == 0 && this.getKeyNumFromXY(mouseX, mouseY) > 0) {
    		//this.keysDown[this.getKeyNumFromXY(mouseX, mouseY)] = true;
    		this.pressKey(this.getKeyNumFromXY(mouseX, mouseY));
    		this.leftClickNote = this.getKeyNumFromXY(mouseX, mouseY);
    ...
    public void pressKey(int index) {
		...
		this.mc.getSoundHandler().playSound(this.pianoSound[index].setEntity(this.piano));
    }
    
    public void unpressKey(int index) {
		this.keysDown[index] = false;
		/*if(index <= 72) {
			this.lingeringSounds.add(this.pianoSound[index]);
		}*/
		this.pianoSound[index].stopPlaying();
		try {this.pianoSound[index] = new PianoSound(this.pianoSound[index]);} catch(Exception e) {}
    }
	...
	public void updateScreen() {
		...
		for(MidiMessage message : this.midiMessage) {
			...
			if(message.getStatus() == 144) {
	    		...
	    		try {this.pressKey(MathHelper.clamp(message.getMessage()[1] - 20, 1, 88));} catch(Exception e) {}
	    		...
	    		this.unpressKey(MathHelper.clamp(message.getMessage()[1] - 20, 1, 88));
			}
		}
