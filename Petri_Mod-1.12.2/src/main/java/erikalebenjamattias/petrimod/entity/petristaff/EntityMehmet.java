...
public class EntityMehmet extends EntityCreature {
	...
	public boolean isEnclosedRoom(BlockPos pos) {
		...
		return list.size() == 16200 && this.world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(pos).grow(16), new Predicate() {
			/*public boolean isPlayerNotSpectating(EntityPlayer player) {
				return !player.isSpectator();
			}*/
			public boolean apply(Object player) {
				return !((EntityPlayer)player).isSpectator();
			}
		}).size() == 0;
	}
